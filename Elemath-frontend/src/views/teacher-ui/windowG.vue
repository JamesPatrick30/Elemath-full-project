                    ...
                    <div class="thead">
                        ...
                        <div class="th-con">
                            ...
                            <div class="th" v-for="(quiz, index) in mazLength(students[0]?.quiz)" :key="quiz.quizId">
                                ...
                                <p v-else-if="quiz.mode === 'WINDOWCARD MODE'">Window Card {{ quiz.windowCardCount }}</p>
            ...
            try {
                ...
                if (res.data && Array.isArray(res.data.students) ) {
                    ...
                    this.students = this.students.map(student =>({
                        name:student.name,
                        quiz:student.quiz.filter(quiz => quiz.mode === 'WINDOWCARD MODE').map(quiz => {
                            windowCardCount++;
                            return {...quiz, windowCardCount};
                        })
                    }));
                    console.log('students data modified : ' + JSON.stringify(this.students));
